...
const JobSearch: React.FC<Props> = ({ onBack, selectedJobLink, onClearSelected, onNotifyJobs }) => {
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedCategory, setSelectedCategory] = useState<string>("All");
    const [modalOpen, setModalOpen] = useState(false);
    const [modalJob, setModalJob] = useState<JobSource | { url: string; name?: string; description?: string } | null>(null);
    ...
    React.useEffect(() => {
        if (selectedJobLink) {
            const found = JOB_SOURCES.find(j => j.url === selectedJobLink || selectedJobLink?.startsWith(j.url));
            if (found) setModalJob(found);
            else setModalJob({ url: selectedJobLink, name: 'Job Opportunity', description: 'Open the link to view job details.' });
            setModalOpen(true);
        }
    }, [selectedJobLink]);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-4 bg-white p-8 rounded-xl shadow-sm border border-slate-200">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => {
                                const sample = JOB_SOURCES.slice(0, 5).map(j => ({ title: j.name, company: j.name, link: j.url }));
                                onNotifyJobs && onNotifyJobs(sample);
                            }}
                            className="text-sm bg-white border border-slate-200 px-3 py-1 rounded text-slate-700 hover:bg-slate-50"
                        >
                            Notify Latest Jobs
                        </button>
                ...
                <p className="text-slate-600">
                    Curated list of <strong>{JOB_SOURCES.length}</strong> official career pages. Stop searching randomly apply directly to the source.
                </p>
                ...
                <div className="flex flex-col md:flex-row gap-4 mt-2">
                    <div className="relative flex-1">
                        ...
                        <input type="text" placeholder="Search (e.g., Google, Internship, Remote)..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#2f8d46] focus:border-transparent outline-none" />
                    </div>
                    <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${selectedCategory === cat ? 'bg-[#2f8d46] text-white shadow-md' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                                {cat}
                            </button>
                        ))}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {filteredJobs.length > 0 ? filteredJobs.map((job, idx) => (
                    <div key={idx} className="bg-white rounded-lg border border-slate-200 overflow-hidden hover:shadow-lg transition-all flex flex-col group relative">
                        <div className={`h-1.5 ${job.category === 'MNC' ? 'bg-blue-600' : job.category === 'Service-Based' ? 'bg-purple-600' : job.category === 'Startup' ? 'bg-orange-500' : job.category === 'Banking' ? 'bg-emerald-600' : job.category === 'Remote' ? 'bg-cyan-500' : job.category === 'Internship' ? 'bg-yellow-500' : 'bg-slate-500'}`}></div>
                        <div className="p-5 flex-1 flex flex-col">
                            <div className="flex justify-between items-start mb-2"><span className={`text-[10px] px-2 py-0.5 rounded uppercase tracking-wide font-bold border ${job.category === 'MNC' ? 'bg-blue-50 text-blue-700 border-blue-100' : job.category === 'Service-Based' ? 'bg-purple-50 text-purple-700 border-purple-100' : job.category === 'Startup' ? 'bg-orange-50 text-orange-700 border-orange-100' : job.category === 'Banking' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : job.category === 'Remote' ? 'bg-cyan-50 text-cyan-700 border-cyan-100' : job.category === 'Internship' ? 'bg-yellow-50 text-yellow-700 border-yellow-100' : 'bg-slate-50 text-slate-700 border-slate-100'}`}>{job.category}</span></div>
                            ...
                            <div className="mt-auto"><a href={job.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full bg-slate-50 hover:bg-[#2f8d46] hover:text-white text-slate-700 border border-slate-200 hover:border-[#2f8d46] font-bold py-2 rounded transition-all text-xs">Visit Career Page ↗</a></div>
                        </div>
                    </div>
                )) : (
                    <div className="col-span-full text-center py-12">
                        <p className="text-slate-500 text-lg">No companies found matching "{searchTerm}".</p>
                        <button onClick={() => {setSearchTerm(""); setSelectedCategory("All");}} className="mt-2 text-[#2f8d46] font-bold hover:underline">Clear Filters</button>
                    </div>
                )}
            </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-xl p-6 text-center shadow-sm">
                ...
                <p className="text-blue-800 text-sm max-w-3xl mx-auto"><strong>ATS Tip:</strong> 70% of resumes are rejected by bots. When applying on these portals, ensure your resume keywords match the Job Description (JD). Use the <strong>"Analyze Resume"</strong> tool in this app before applying!</p>
            </div>

            {modalOpen && modalJob && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    <div className="bg-white p-4 rounded">Job details — opening external page is supported.</div>
                </div>
            )}
        </div>
    );
